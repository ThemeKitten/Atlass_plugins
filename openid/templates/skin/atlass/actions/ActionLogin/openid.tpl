...
<div class="openid-block">
	<h1 class="openid-header"><img src="{$aTemplateWebPathPlugin.openid}img/openid.png" alt="openid" class="openid-img" title="{$aLang.openid}" alt="{$aLang.openid}"/></h1>
	
	<form style="margin-top:20px;" method="post" action="{router page='login'}openid/enter/" name="fopenid" id="openid_form">
		<div style="overflow: hidden; zoom: 1;">
			<input type="text" style="float: left" class="input-text input-width-300" maxlength="255" name="open_login" id="open_login" />
			<input type="hidden" name="submit_open_login" id="submit_open_login_hidden" value="go"/>
			<input type="hidden" value="{$_aRequest.return}" name="return" />
			<a href="#" style="padding:11px 9px;margin-left:10px;" class="button" onclick="getEl('openid_form').submit(); return false;">{$aLang.plugin.openid.enter}</a>
		</div>
