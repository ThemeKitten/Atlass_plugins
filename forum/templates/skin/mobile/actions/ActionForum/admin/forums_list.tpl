					...
					<div class="mb-20">
						...
						{foreach from=$aForumsTree item=aItem}
							...
							<li id="forum-{$oForum->getId()}"{if $aItem.level == 0} class="head"{else} style="margin-left:{$aItem.level*25}px"{/if}>
								...
								{if $aItem.level > 0}
									<span class="js-forum-moder-toogler js-tip-help fl-r icon-white icon-plus-sign" id="toggler-moder-list-{$oForum->getId()}" title="{$aLang.plugin.forum.moderators_list}"></span>
