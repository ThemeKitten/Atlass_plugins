...
ls.forum.admin = (function ($) {
	...
	this.initModerToggler = function() {
		$('.js-forum-moder-toogler').click(function() {
			...
			if (list.is(":visible")) {
				$(this).addClass('icon-white icon-plus-sign').removeClass('icon-white icon-minus-sign');
				$(list).slideUp();
			} else {
				$(this).removeClass('icon-white icon-plus-sign').addClass('icon-white icon-minus-sign');
				$(list).slideDown();
			}
			return false;
		});
